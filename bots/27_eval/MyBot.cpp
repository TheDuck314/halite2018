...
//                    if (false) {  // can be useful to turn this off for testing
    ...
    /*int estimate_mining_time(Vec start_square, int start_halite, const PosMap<double> &effective_halite)
    {
        const bool debug = Game::turn < 10;
        if (debug) Log::flog(start_square, "estimate_mining_time start=%s ship_halite=%d", +start_square.toString(), start_halite);
        // heap of squares sorted by halite, max is top
        auto cmp = [&effective_halite](Vec a, Vec b){ return effective_halite(a) < effective_halite(b); };
        priority_queue<Vec, vector<Vec>, decltype(cmp)> square_heap(cmp);
        set<Vec> already_added;

        already_added.insert(start_square);
        Vec current_square = start_square;
        double current_square_halite = effective_halite(current_square);

        for (int d = 0; d < 4; ++d) {
            const Vec adj = grid.add(current_square, (Direction)d);
            square_heap.push(adj);
            already_added.insert(adj);
        }

        Vec next_square = square_heap.top();
        double next_square_halite = effective_halite(next_square);

        double halite_needed = Constants::MAX_HALITE - start_halite;
        int turns = 0;        
        while (true) {
            turns += 1;
            const double amt_mined = current_square_halite / 4;
            halite_needed -= amt_mined;
            current_square_halite -= amt_mined;
            if (debug) Log::flog(start_square, "turns=%d  amt_mined = %f  current_square_halite = %f", turns, amt_mined, current_square_halite);
            if (halite_needed <= 50) break;

            // stay on this square unless the next square has way more halite
            if (current_square_halite * 3.0 > next_square_halite) continue;

            // the next square has way more halite, so we're going to move to it
            turns += 1;  // we'll spend a turn moving
            current_square = next_square;
            current_square_halite = next_square_halite;
            if (debug) Log::flog(start_square, "turns=%d  moving to %s", turns, +current_square.toString());
            square_heap.pop();
            next_square = square_heap.top();
            next_square_halite = effective_halite(next_square);

            // add all the neighbors of the new current square to the heap
            for (int d = 0; d < 4; ++d) {
                const Vec adj = grid.add(current_square, (Direction)d);
                if (already_added.count(adj)) continue;
                square_heap.push(adj);
                already_added.insert(adj);
            }
        }

        if (debug) Log::flog(start_square, "estimate_mining_time DONE: turns=%d", turns);
        return turns;
    }*/

    double estimate_mining_time(Vec start_square, int ship_halite, const PosMap<double> &effective_halite)
    {        
        double halite_needed = Constants::MAX_HALITE - ship_halite;
        const double start_square_halite = effective_halite(start_square);

        double avg_nearby_halite = 10.0; // to prevent some problems
        for (int d = 0; d < 4; ++d) {
            avg_nearby_halite += effective_halite(grid.add(start_square, (Direction)d));
        }
        avg_nearby_halite /= 4;

        const double start_square_move_on_thresh = 0.33 * avg_nearby_halite;
        const double max_mined_from_start_square = start_square_halite - start_square_move_on_thresh;
//        Log::flog(start_square, "ship_halite=%d halite_needed=%f start_square_halite=%f avg_nearby_halite=%f",
//            ship_halite, halite_needed, start_square_halite, avg_nearby_halite);
//        Log::flog(start_square, "start_square_move_on_thresh=%f max_mined_from_start_square=%f",
//                  start_square_move_on_thresh, max_mined_from_start_square);
        if (max_mined_from_start_square > halite_needed) {
            // we will fill up from the start square
            const double final_start_square_halite = start_square_halite - halite_needed;
//            Log::flog(start_square, "case A, final_start_square_halite=%f", final_start_square_halite);
            // we'll get down to that halite after T turns where 0.75^T = final_start_square_halite / start_square_halite
            return log(final_start_square_halite / start_square_halite) / log(0.75);
        } else {
            // we will mine the max amount from the start square, then move on to the neighborhood
            double start_square_turns;
            if (start_square_halite < start_square_move_on_thresh) {
//               Log::flog(start_square, "AAA");
                start_square_turns = 0.0;
            } else {
//               Log::flog(start_square, "BBB");
                start_square_turns = log(start_square_move_on_thresh / start_square_halite) / log(0.75);
                halite_needed -= max_mined_from_start_square;
            }
            // now let's assume each square has avg_nearby_halite, and we spend 5 turns for each
            // square (1 to move onto it and 4 to mine), so we extract roughly 0.65 of avg_nearby_halite
            // every 5 turns, so we get 0.13 of avg_nearby_halite per turn, so it takes this many turns
            const double other_square_turns = halite_needed / (0.13 * avg_nearby_halite);  // TUNE!
//            Log::flog(start_square, "case B, start_square_turns=%f new halite_needed=%f other_square_turns=%f", 
//                      start_square_turns, halite_needed, other_square_turns);
            return start_square_turns + other_square_turns;
        }
    }

    double estimate_trip_time(Ship ship, const PosMap<double> &effective_halite, bool returning, Vec flog_pos)
    {
        if (returning) {
            // MISSING:
            // - preferring to return to rich dropoffs
            // - using bfs distance to dropoffs
            // - ability to return to planned_dropoff_pos
            return grid.smallest_dist(ship.pos, Game::me->structures);
        }
        ...
        for (Vec pos : grid.positions) {
            ...
            const double turns_to_fill_up = estimate_mining_time(pos, ship.halite, effective_halite);
        ...
        //Log::flog(ship.pos, "when %d is here: best_tgt = %s", ship.id, +best_tgt.toString());

        Log::flog(flog_pos, "estimate_trip_time %s -> %f", +ship.toString(), best_total_time);
    ...
    {
        Direction best_move;
        double best_est_time;

        string toString() { return stringf("SearchResult(best_move=%s, best_est_time=%f)", +desc(best_move), best_est_time); }
    };

    SearchResult do_search(Ship ship, int depth, bool returning, PosMap<double> &effective_halite, const PosMap<bool> &impassable, Vec flog_pos)
    {
        const int move_cost = grid(ship.pos).halite / 4;  // true halite, not effective

        double best_est_time = 1e99;
        Direction best_move = Direction::Still;

        for (int d_int = 0; d_int < 5; ++d_int) {
            const Direction d = (Direction)d_int;
            if (d != Direction::Still && ship.halite < move_cost) continue;
            const Vec dest = grid.add(ship.pos, d);
            if (d != Direction::Still && impassable(dest)) continue;

            // Save some state to be restored later
            const Ship saved_ship = ship;
            const double saved_here_effective_halite = effective_halite(ship.pos);
            const int saved_here_true_halite = grid(ship.pos).halite;
            const bool saved_returning = returning;

            // compute the state of the ship, effective_halite, and true halite after this move
            if (d == Direction::Still) {
                const int halite_room = Constants::MAX_HALITE - ship.halite;
                int effective_amt_mined = min(halite_room, (int)(saved_here_effective_halite / 4));
                ship.halite += effective_amt_mined;
                effective_halite(ship.pos) -= effective_amt_mined;
                int true_amt_mined = min(halite_room, saved_here_true_halite / 4);  // NOTE: wrong rounding
                grid(ship.pos).halite -= true_amt_mined;  // DANGEROUS! fix this
                if (ship.halite >= RETURN_HALITE_THRESH.get(950)) returning = true;
            } else {  // non-Still move
                ship.halite -= move_cost;
                ship.pos = dest;
            }

            // evaluate this new state of the world
            double est_time;
            // don't recurse if we've hit our depth limit, or if we're returning and reached a dropoff
            if (depth == 1 || (returning && contains(Game::me->structures, ship.pos))) {
                est_time = 1.0 + estimate_trip_time(ship, effective_halite, returning, flog_pos);
            } else {
                est_time = 1.0 + do_search(ship, depth - 1, returning, effective_halite, impassable, flog_pos).best_est_time;
            }
            Log::flog(flog_pos, "depth %d %s dir %s est_time %f", depth, +ship.toString(), +desc(d), est_time);

            if (est_time < best_est_time) {
                best_est_time = est_time;
                best_move = d;
            }

            // restore the state of the world from before this move
            ship = saved_ship;
            effective_halite(ship.pos) = saved_here_effective_halite;
            grid(ship.pos).halite = saved_here_true_halite;
            returning = saved_returning;
        }

        return {best_move, best_est_time};
    }
    ...
    void plan_mining()
    {
        ...
        for (Ship ship : Game::me->ships) {
            ...
            } else if (ship.halite >= RETURN_HALITE_THRESH.get(950)) {
                returning_ship_ids.insert(ship.id);
            }
        }

        Log::flog(Game::me->shipyard, "plan_mining start miners.size() = %d", (int)miners.size());
        ...
        for (Ship ship : miners) {
            // first attempt: evaluate each of the 5 possible actions, and pick the best

            const PosMap<bool> &impassable = sid_to_impassable.at(ship.id);
            const bool returning = returning_ship_ids.count(ship.id) > 0;
            Log::flog(ship.pos, "returning = %d", returning);

            
            const int depth = 5;
            SearchResult result = do_search(ship, depth, returning, effective_halite, impassable, ship.pos);

            const Vec dest = grid.add(ship.pos, result.best_move);
            Log::flog(ship.pos, "PLAN_MINING TOP LEVEL result: %s", +result.toString());
            plan(ship, dest, Purpose::Mine);

            Log::flog(ship.pos, "estimate_mining_time of here: %f", estimate_mining_time(ship.pos, ship.halite, effective_halite));

            //estimate_mining_time(ship, effective_halite);
            estimate_trip_time(ship, effective_halite, returning, ship.pos);
        }
    }
