...
struct Bot {
    ...
    bool want_more_ships()
    {
        ...
        if (Game::num_players != 2) {
            ...
            if (halite_left < MIN_HALITE_PER_SHIP_TO_SPAWN_4P.get(1500) * (int)Game::me->ships.size()) return false;  // TUNE
        } else { // two players
...
//            if (Game::turn < 200) Log::flog(v, "return_richness_bonus = %f", return_richness_bonus(v));
        ...
        while (!q.empty()) {
            ...
            for (Vec pos : grid.positions) {
                ...
                /*if (ship.id == 2) {
                    Log::flog(pos, "mine_rate = %f", mine_rate);
                    Log::flog(pos, "turns_to_fill_up = %f", turns_to_fill_up);
                    Log::flog(pos, "dist_from_base = %d", dist_from_base);
                    Log::flog(pos, "dist_from_me = %d", dist_from_me);
                    Log::flog(pos, "total_time = %f", total_time);
                    Log::flog(pos, "overall_halite_rate = %f", overall_halite_rate);
                }*/
