...
void plan_mining(const vector<Ship> &miners)
{
    ...
    // halite in the enemy ship on each square, if any
    PosMap<int> enemy_ship_halite(0);
    for (Ship enemy_ship : Game::enemy_ships) {
        enemy_ship_halite(enemy_ship.pos) = enemy_ship.halite;
    }
    ...
    while (!q.empty()) {
        ...
        for (int x = 0; x < grid.width; ++x) {
            for (int y = 0; y < grid.height; ++y) {
                ...
                double halite = grid(pos).halite;
                if (halite <= 0) continue;
                if (inspired(pos)) halite *= (1 + Constants::INSPIRED_BONUS_MULTIPLIER);

                // If there's an enemy ship here and this is still a valid target, count the ship's halite
                // as a bonus to the square's halite. That'll encourage us to seek out and ram
                // juicy targets.
                halite += 0.5 * enemy_ship_halite(pos);
