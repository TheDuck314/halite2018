...
enum class Purpose
{
    ...
    EvadeReturner = 1,  // getting out of the way of someone with the Return purpose
    Return = 2,         // bringing halite back to base
    Mine = 3,           // getting halite
};
...
/*vector<Command> resolve_plans_into_commands(vector<Command> &commands)
{
...
void make_final_collapse_commands(vector<Command> &commands)
{
    ...
    for (Plan p : plans) {
        ...
        if (grid.dist(p.ship.pos, structure) <= 1) {
            Direction d = grid.adj_dir(p.ship.pos, structure);
            commands.push_back(Command::move(p.ship, d));
        } else {
...
void resolve_plans_into_commands(vector<Command> &commands)
{
    ...
    // could move to next turn
    PosSet avoid_positions(false);
    for (const Ship &enemy_ship : Game::enemy_ships) {
        // we need to ignore enemy ships that are on top of our structures
        // or they'll block us from delivering. it's fine to ram them.
...
void plan_mining(const vector<Ship> &miners)
{
    ...
    PosSet inspired(false);
    for (Vec pos : grid.positions) {
        inspired(pos) = nearby_enemy_ship_count(pos) >= Constants::INSPIRATION_SHIP_COUNT;
    }
    ...
    while (!q.empty()) {
        ...
        for (int x = 0; x < grid.width; ++x) {
            for (int y = 0; y < grid.height; ++y) {
                Vec pos{x, y};
                if (invalid_tgts(pos)) continue;
                int halite = grid(pos).halite;
                if (halite <= 0) continue;
                if (inspired(pos)) halite *= (1 + Constants::INSPIRED_BONUS_MULTIPLIER);
                ...
                // get (cell halite) / EXTRACT_RATIO halite per turn. And let's assume we can
                // mine at a similar rate from nearby cells. Then it'll take this long to fill up:
                double mine_rate = halite * Constants::INV_EXTRACT_RATIO;
                ...
                // Let's say your mining rate is 50% slower that otherwise at low halite
                const double efficiency = 0.5 + 0.5 * max(0, min(Constants::MAX_HALITE, halite)) / (double)Constants::MAX_HALITE;
                const double turns_to_fill_up = Constants::MAX_HALITE / (mine_rate * efficiency);
                // note the 5* fudge factor:
                const double overall_rate = (mine_rate * turns_to_fill_up) / (turns_to_fill_up + dist_from_base + 2 * dist_from_me);
    ...
    for (Vec tgt : grid.positions) {
        ...
        Log::flog(ship, "final mining tgt: %s (inspired = %d)", +tgt.toString(), inspired(tgt));
...
int consider_making_dropoff(vector<Command> &commands)
{
    ...
    if (Game::me->ships.size() < 20 * (1 + Game::me->dropoffs.size())) {
...
/*int consider_making_dropoff(vector<Command> &commands)
{
    ...
    if (best_id != -1) {
        commands.push_back(Command::construct(best_id));
    }
...
vector<Command> turn()
{
    ...
    for (Plan &p : plans) {
        if (grid(p.ship.pos).halite / Constants::MOVE_COST_RATIO > p.ship.halite) {
