...
string squares_string(const vector<Vec> &squares) {
    string ret;
    for (unsigned i = 0; i < squares.size(); ++i) {
        if (i > 0) ret += ", ";
        ret += squares[i].toString();
    }
    return ret;
}
...
//                    if (false) {  // can be useful to turn this off for testing
    ...
    void consider_making_dropoff(vector<Command> &commands)
    {
        ...
        for (Vec pos : grid.positions) {
            ...
            double dist_factor = 1.0 - DROPOFF_PENALTY_FRAC_PER_DIST.get(0.05) * dist_from_base;  // TUNE
...
/*            if (ship.halite < 100) {
    ...
    struct SimPathResult
    {
        int halite_delivered;
        int turns_taken;
    };

    double score_path(SimPathResult result)
    {
        //return result.halite_delivered / (double)result.turns_taken;  // fixme
        double ret = result.halite_delivered * exp(-0.05 * result.turns_taken);
        if (result.halite_delivered < 900) ret *= 0.5;
        return ret;
    }

    int max_halite_along_path(Ship ship, const vector<Vec> &squares)
    {
        // I don't like this :(
        int ret = grid(ship.pos).halite;
        for (Vec tgt : squares) {
            while (ship.pos != tgt) {
                const Direction d = grid.approach_dirs(ship.pos, tgt)[0];
                ship.pos = grid.add(ship.pos, d);
                const int here_halite = grid(ship.pos).halite;
                if (here_halite > ret) ret = here_halite;
            }
        }
        return ret;
    }

    enum class UpdateEffectiveHalite { No, Yes };

    // path_squares should include the final structure
    // FIXME we're going to neglect inspiration for now, as well as any other 
    // artificial modifiers to effective halite
    // FIXME inspiration multiplies effective halite for the purposes of mining, but
    // not for move cost. we can't account for this with a single effective_halite map
    SimPathResult simulate_path(Ship ship, const vector<Vec> &path_squares, int min_halite_to_mine, 
                                PosMap<int> &effective_halite, 
                                UpdateEffectiveHalite update_effective_halite,
                                bool debug = false)
    {
        const Vec flog_pos = ship.pos;

        SimPathResult result;
        result.turns_taken = 1;  // silly thing to avoid a divide by zero
        if (debug) {
            Log::flog(flog_pos, "simulating path ship=%s<br/>path_squares=%s<br/>min_halite_to_mine=%d", 
                      +ship.toString(), +squares_string(path_squares), min_halite_to_mine);
        }

        int here_halite = effective_halite(ship.pos);

        map<Vec, int> up_to_date_halite;  // remember when we've mined a square down. TODO: optimizations?
        bool mined_this_square = false;

        for (Vec tgt : path_squares) {
            // move along until we get to the target
            if (debug) {
                Log::flog(flog_pos, "starting towards tgt %s", +tgt.toString());
            }
            while (ship.pos != tgt) {
                result.turns_taken += 1;
                const int move_cost = here_halite / 10;
                if (ship.halite < Constants::MAX_HALITE && (here_halite >= min_halite_to_mine || ship.halite < move_cost)) {
                    // stop here to mine
                    const int amt_mined = min(Constants::MAX_HALITE - ship.halite, (here_halite + 3) / 4);
                    here_halite -= amt_mined;
                    ship.halite += amt_mined;
                    mined_this_square = true;
                    if (debug) {
                        Log::flog(flog_pos, "staying; now ship.halite=%d here_halite=%d", ship.halite, here_halite);
                    }
                } else {
                    // move on from this square
                    ship.halite -= move_cost;

                    // remember the amount of halite left on this square, if we changed it
                    if (mined_this_square) {
                        up_to_date_halite[ship.pos] = here_halite;
                    }

                    // TODO: currently doing a dumb direct-approach thing. But ideally we probably want to
                    // do min-halite pathfinding and cache the results somehow
                    const Direction d = grid.approach_dirs(ship.pos, tgt)[0];
                    ship.pos = grid.add(ship.pos, d);

                    // if we've touched the new square, look up how much halite is left on the new squrae
                    // in this sim. otherwise just use the halite value from effective_halite
                    auto it = up_to_date_halite.find(ship.pos);
                    if (it != up_to_date_halite.end()) {
                        here_halite = it->second;
                    } else {
                        here_halite = effective_halite(ship.pos);
                    }

                    mined_this_square = false;

                    if (debug) {
                        Log::flog(flog_pos, "moving onto %s which has %d halite", +ship.pos.toString(), here_halite);
                    }
                }
            }
        }

        result.halite_delivered = ship.halite;
        if (debug) {
            Log::flog(flog_pos, "sim results turns_taken=%d halite_delivered=%d score=%f", 
                      result.turns_taken, result.halite_delivered, score_path(result));
        }

        if (update_effective_halite == UpdateEffectiveHalite::Yes) {
            for (const pair<Vec, int> &pos_new_halite : up_to_date_halite) {
                Log::flog(flog_pos, "updating %s halite %d -> %d", +pos_new_halite.first.toString(), effective_halite(pos_new_halite.first),
                          pos_new_halite.second);
                effective_halite(pos_new_halite.first) = pos_new_halite.second;
            }
        }
    ...
    double simulate_and_score_path(Ship ship, const vector<Vec> &path_squares, int min_halite_to_mine, PosMap<int> &effective_halite)
    {
        return score_path(simulate_path(ship, path_squares, min_halite_to_mine, effective_halite, UpdateEffectiveHalite::No));
    }

    struct ScoredPath
    {
        vector<Vec> squares;
        int min_halite_to_mine = -99;
        double score = -99;

        string toString()
        {
            return stringf("ScoredPath(score=%f, min_halite_to_mine=%d, squares={%s})", score, min_halite_to_mine, +squares_string(squares));
        }

        bool operator<(const ScoredPath &rhs) { return score < rhs.score; }
    };

    void optimize_stopping_strategy(Ship ship, ScoredPath &path, PosMap<int> &effective_halite)
    {
        // as a baseline, try never stopping unless we can't move:
        path.min_halite_to_mine = 9999;
        path.score = simulate_and_score_path(ship, path.squares, path.min_halite_to_mine, effective_halite);

        // TODO: account for inspiration?
        int max_thresh = max_halite_along_path(ship, path.squares);

        // currently: grid search. could be smarter?
        int increment = max(1, max_thresh / 8);  // need max(1, ) to avoid infinite loop.
        for (int thresh_to_try = increment; thresh_to_try <= max_thresh; thresh_to_try += increment) {
            const double score = simulate_and_score_path(ship, path.squares, thresh_to_try, effective_halite);
            if (score > path.score) {
                path.score = score;
                path.min_halite_to_mine = thresh_to_try;
            }
        }
        //Log::flog(ship.pos, "path: %s  (max_thresh = %d)", +path.toString(), max_thresh);
        /*if (path.squares.size() == 1) {
            Log::flog(ship.pos, "doing a debug sim b/c path.size() == 1:");
            simulate_path(ship, path.squares, path.min_halite_to_mine, true);
        }*/
    }

    void try_add_new_path(Ship ship, ScoredPath &new_path, list<ScoredPath> &paths, 
                          unordered_set<int> &known_path_hashes, PosMap<int> &effective_halite)
    {
        // first, discard the path if it's one we've already tried
        const int path_hash = path_hasher.hash(new_path.squares);
        if (known_path_hashes.count(path_hash)) {
            //Log::flog(ship.pos, "already tried: %s", +new_path.toString());
            return;
        }

        // otherwise, score the path and put it in the list
        optimize_stopping_strategy(ship, new_path, effective_halite);
        paths.push_back(new_path);
        known_path_hashes.insert(path_hash);
        //Log::flog(ship.pos, "added path with hash %d: %s", path_hash, +new_path.toString());
    }

    void make_basic_scored_path(Ship ship, Vec dest, list<ScoredPath> &paths, 
                                unordered_set<int> &known_path_hashes, PosMap<int> &effective_halite)
    {
        ScoredPath ret;
        ret.squares.push_back(dest);
        //Log::flog(ship.pos, "basic scored path: %s", +ret.toString());
        try_add_new_path(ship, ret, paths, known_path_hashes, effective_halite);
    }

    /*void add_mutated_paths_delete(Ship ship, const ScoredPath &seed, list<ScoredPath> &new_paths)
    {
        // note: careful not to delete the final structure
        for (unsigned index_to_delete = 0; index_to_delete < seed.squares.size() - 1; ++index_to_delete) {
            ScoredPath new_path;
            for (unsigned i = 0; i < seed.squares.size(); ++i) {
                if (i != index_to_delete) {
                    new_path.squares.push_back(seed.squares[i]);
                }
            }
            optimize_stopping_strategy(ship, new_path);
            new_paths.push_back(new_path);
        }
    }*/

    /*void add_mutated_paths_swap(Ship ship, const ScoredPath &seed, list<ScoredPath> &new_paths)
    {
        // note: careful not to swap the final structure
        for (unsigned swap_i = 0; swap_i < seed.squares.size() - 2; ++swap_i) {
            for (unsigned swap_j = swap_i + 1; swap_j < seed.squares.size() - 1; ++swap_j) {
                ScoredPath new_path;
                new_path.squares = seed.squares;
                swap(new_path[swap_i], new_path[swap_j]);
                optimize_stopping_strategy(ship, new_path);
                new_paths.push_back(new_path);
            }
        }
    ...
    vector<Vec> guess_good_mining_squares_between(Ship ship, const PosMap<int> &effective_halite, Vec start, Vec end)
    {
        // TODO: probably halite_needed is wrong when this is in the middle of a longer path
        const double halite_needed = Constants::MAX_HALITE - ship.halite;
        ...
        // track the N best squares
        const int heap_size = 5;
        using SquareAndTime = pair<Vec, double>;
        auto cmp = [](SquareAndTime a, SquareAndTime b) { return a.second < b.second; };
        priority_queue<SquareAndTime, vector<SquareAndTime>, decltype(cmp)> best_heap(cmp);

        // pick the best target.
        double best_total_time = 1e99;
        Vec best_tgt = ship.pos;
        for (Vec pos : grid.positions) {
            if (impassable(pos)) continue;
            double halite = effective_halite(pos);
            ...
            // THIS FUNCTION DIFFERS FROM OLD PLAN_MINING IN THAT IT TRIES NOT TO SELECT THE SHIP'S OWN SQUARE!!
            if (ship.pos == pos) continue;

            const int dist_from_start = grid.dist(pos, start);
            const int dist_from_end = grid.dist(pos, end);
            ...
            const double total_time = turns_to_fill_up 
                                    + dist_from_end
                                    + MINING_DIST_FROM_ME_MULT.get(2.0) * dist_from_start;  // TUNE. MINING_DIST_FROM_ME_MULT 1.0 is worse than 2.0, 3.0 about the same as 1.0

            //Log::flog(pos, "total_time = %f", total_time);
            ...
            best_heap.push({pos, total_time});
            if (best_heap.size() > heap_size) best_heap.pop();
        }

        vector<Vec> ret;
        while (!best_heap.empty()) {
            SquareAndTime st = best_heap.top();
            ret.push_back(st.first);
            best_heap.pop();
            //Log::flog_color(st.first, 0, 0, 255, "top %d!", heap_size);
        }
        return ret;
    }


    vector<Vec> propose_squares(Vec start, Vec end, const vector<Vec> &used_squares)
    {
        // here's where you need to actually do some work!
        // figure out how to import our smarts from plan_mining!
        vector<Vec> ret;
        // this is a dumb placeholder, change as soon as possible!
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                Vec v = grid.add(start, dx, dy);
                if (!contains(used_squares, v)) {
                    ret.push_back(v);
                }
            }
        }
    ...
    vector<Vec> insert_square(const vector<Vec> squares, Vec new_square, unsigned before_i)
    {
        vector<Vec> ret;
        for (unsigned i = 0; i < squares.size(); ++i) {
            if (i == before_i) ret.push_back(new_square);
            ret.push_back(squares[i]);
        }
        return ret;
    }

    void add_mutated_paths_insert(Ship ship, const ScoredPath &seed, list<ScoredPath> &paths,
                                  unordered_set<int> &known_path_hashes, PosMap<int> &effective_halite)
    {
        // careful not to insert past the structure at the end
        vector<Vec> used_squares(seed.squares);
        used_squares.push_back(ship.pos);
        //Log::flog(ship.pos, "add_mutated_paths_insert start");
        for (unsigned before_i = 0; before_i < seed.squares.size(); ++before_i) {
            const Vec ins_start = (before_i == 0 ? ship.pos : seed.squares.at(before_i - 1));
            const Vec ins_end = seed.squares.at(before_i);
            const vector<Vec> proposals = guess_good_mining_squares_between(ship, effective_halite, ins_start, ins_end);
            for (Vec proposal : proposals) {
                ScoredPath new_path;
                new_path.squares = insert_square(seed.squares, proposal, before_i);
                //Log::flog(ship.pos, "new_path: %s", +new_path.toString());
                try_add_new_path(ship, new_path, paths, known_path_hashes, effective_halite);
            }
        }
        //Log::flog(ship.pos, "add_mutated_paths_insert end");
    }

    /*void add_mutated_paths_shift(Ship ship, const ScoredPath &seed, list<ScoredPath> &new_paths)
    {
        // like insert by modify an existing square instead of inserting a new one
    }*/

    void add_mutated_paths(Ship ship, const ScoredPath &seed, list<ScoredPath> &new_paths,
                           unordered_set<int> &known_path_hashes, PosMap<int> &effective_halite)
    {
        //add_mutated_paths_delete(ship, seed, new_paths);  // drop a square (but not the final structure)
        add_mutated_paths_insert(ship, seed, new_paths, known_path_hashes, effective_halite);  // insert a new square anywhere but at the end
        //add_mutated_paths_swap(ship, seed, new_paths);    // swap two squares (but don't mess with the last one)
        //add_mutated_paths_shift(ship, seed, new_paths);   // shift one of the squares
    }

    void add_initial_seed_paths(Ship ship, list<ScoredPath> &paths, unordered_set<int> &known_path_hashes,
                                PosMap<int> &effective_halite)
    {
        // a smarter seed
        for (Vec end : Game::me->structures) {  // TODO: slow when we have many structures/
            const vector<Vec> good_mining_squares = guess_good_mining_squares_between(ship, effective_halite, ship.pos, end);
            for (Vec v : good_mining_squares) {
                if (v != ship.pos) {  // ship.pos is already implicit in any path
                    ScoredPath seed;
                    seed.squares.push_back(v);
                    seed.squares.push_back(end);
                    try_add_new_path(ship, seed, paths, known_path_hashes, effective_halite);
                }
            }
        }

        // as very simple candidate paths which just go to each structure
        for (Vec structure : Game::me->structures) {
            make_basic_scored_path(ship, structure, paths, known_path_hashes, effective_halite);
        }
    }

    ScoredPath find_good_path(Ship ship, PosMap<int> &effective_halite)
    {
        list<ScoredPath> paths;
        unordered_set<int> known_path_hashes;

        add_initial_seed_paths(ship, paths, known_path_hashes, effective_halite);

        const int max_climb_rounds = 2;
        //const int max_num_paths = 10;

        for (int climb_round = 0; climb_round < max_climb_rounds; ++climb_round) {
            // 1. use our existing paths to generate new paths
            list<ScoredPath> new_paths;
            for (const ScoredPath &path : paths) {
                add_mutated_paths(ship, path, new_paths, known_path_hashes, effective_halite);
            }

            // add the new paths to the big list
            paths.splice(paths.end(), new_paths);

            // 2. eliminate the worst seed paths if we have too many
            //prune_worst_paths(paths, max_num_paths);
        }

        auto max_it = std::max_element(paths.begin(), paths.end());
        return *max_it;
    }

    void awesome_new_mining()
    {
        PosMap<int> effective_halite(0);
        for (Vec v : grid.positions) {
            effective_halite(v) = grid(v).halite;
            // TODO: inspiration, enemy halite
        }
        ...
        // sort from highest halite to lowest halite
        std::sort(miners.begin(), miners.end(), [](auto a, auto b) { return a.halite > b.halite; });
        if (!miners.empty()) {
            Log::flog(miners[0].pos, "I go first!");
            Log::flog(miners[miners.size() - 1].pos, "I go last!");
        }

        for (Ship ship : miners) {
            ScoredPath path = find_good_path(ship, effective_halite);
            // debugging:
            Log::flog(ship.pos, "awesome_new_mining path = %s", +path.toString());
            simulate_path(ship, path.squares, path.min_halite_to_mine, effective_halite, UpdateEffectiveHalite::No, true);

            /*Log::flog(ship.pos, "SIMMING THAT SAME PATH WITH min_halite_to_mine=1000");
            simulate_path(ship, path.squares, 1000, true);*/
            for (unsigned i = 0; i < path.squares.size() - 1; ++i) {
                Log::flog_color(path.squares[i], 255 - i * 40, 0, i * 40, "path.squares[%u]", i);
            }

            /*vector<vector<Vec>> paths_to_compare {
                {{ 7, 17}, { 8, 17}, { 8, 16}},
                {{ 7, 18}, { 8, 17}, { 8, 16}}
            };
            for (unsigned i = 0; i < paths_to_compare.size(); ++i) {
                Log::flog(ship.pos, "for debugging, optimizing path #%u = %s", i, +squares_string(paths_to_compare[i]));
                ScoredPath debug_path;
                debug_path.squares = paths_to_compare[i];
                optimize_stopping_strategy(ship, debug_path);
                Log::flog(ship.pos, "after optimization it's %s", +debug_path.toString());
                Log::flog(ship.pos, "here's a sim:");
                simulate_path(ship, debug_path.squares, debug_path.min_halite_to_mine, true);
            }*/

            // NOT DEBUGGING, THIS IS A REAL THING:
            // pretend we actually do this route and take away the mined halite from each square,
            // so that future turtles know to avoid the squares we went over
            simulate_path(ship, path.squares, path.min_halite_to_mine, effective_halite, UpdateEffectiveHalite::Yes);

            
            bool should_stay = (grid(ship.pos).halite >= path.min_halite_to_mine) || (!can_move(ship));
            if (ship.halite >= Constants::MAX_HALITE) should_stay = false;
            if (should_stay) {
                Log::flog(ship.pos, "awesome_new_mining staying");
                plan(ship, ship.pos, Purpose::Mine);
            } else {
                const Vec tgt = path.squares[0];
                Log::flog(ship.pos, "awesome_new_mining going to first square %s", +tgt.toString());
                plan(ship, tgt, Purpose::Mine);
            }
        }
    }
