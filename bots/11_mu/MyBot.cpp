...
void consider_fleeing()
{
    ...
    for (Ship ship : Game::me->ships) {
        ...
        Log::flog_color(ship.pos, 255, 125, 0, "dist_to_second_closest_enemy = %d", dist_to_second_closest_enemy);

        // be afraid!
        // try to move toward a passable location ranked by ally distance
        Vec best_dest;
        int best_ally_dist = 999999;
        bool found_dest = false;
        for (int d = 0; d < 4; ++d) {
            Vec adj = grid.add(ship.pos, (Direction)d);
            if (impassable(adj)) continue;            
            const int ally_dist = grid.smallest_dist_except(adj, Game::me->ships, ship);
            Log::flog(ship.pos, "%s ally_dist = %d", +adj.toString(), ally_dist);
            if (ally_dist < best_ally_dist && ally_dist > 0) {
